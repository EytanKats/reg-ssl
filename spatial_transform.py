...
class AffineTransformer(torch.nn.Module):
    ...
    def forward(self, src, mat, mode='bilinear'):
        ...
        mat_new = mat/norm
        grid = torch.nn.functional.affine_grid(mat_new, [src.shape[0], 3, src.shape[2], src.shape[3], src.shape[4]])
...
class SpatialTransformer(torch.nn.Module):
    ...
    def forward(self, src, flow, mode='bilinear', padding_mode='zeros'):
        ...
        return new_locs, torch.nn.functional.grid_sample(src, new_locs, mode=mode, padding_mode=padding_mode)
...
class SpatialTransform(object):
    ...
    def augment_spatial(self, data, code_aff=None, code_spa=None, mode='bilinear'):
        ...
        if code_spa is not None:
            aff, data = self.stn(data, code_spa, mode=mode, padding_mode='zeros')
        return aff, data
